...
namespace CataloggerPlugin.StatefullServices
{
	...
	public class MutationsService
	{
		...
		//protected UIDynamicToggle _allMorphsToggle;
		//protected UIDynamicToggle _predefinedMorphsToggle;
		//protected UIDynamicToggle _categoryMorphsToggle;
		...
		protected MVRScript _context;

		public void Init(MVRScript context)
		{
		...
		//public Mutation CaptureCurrentAsset(Atom atom)
		//{
		//	try
		//	{
		//		CustomUnityAssetLoader customAsset = atom.GetStorableByID("asset") as CustomUnityAssetLoader;
		//		var assetUrl = customAsset.GetUrlParamValue("assetUrl");
		//		var assetName = customAsset.GetStringChooserParamValue("assetName");
		//		var newMutation = new Mutation();
		//		newMutation.AtomName = atom.name;
		//		newMutation.AtomType = atom.type;
		//		newMutation.AssetUrl = assetUrl;
		//		newMutation.AssetName = assetName;
		//		return newMutation;
		//	}
		//	catch (Exception exc)
		//	{
		...
		//public Mutation CaptureCurrentSelectedAtom()
		//{
		//	try
		//	{
		//		var selectedAtom = SuperController.singleton.GetSelectedAtom();
		//		if (selectedAtom == null) {
		//			SuperController.LogMessage("Please select an atom!");
		//			SuperController.LogError("Please select an atom!");
		//			return new Mutation();
		//			//throw new Exception("No atom selected!");
		//		}
		//		var newMutation = new Mutation();
		//		newMutation.AtomName = selectedAtom.name;
		//		newMutation.AtomType = selectedAtom.type;
		//		return newMutation;
		//	}
		//	catch (Exception exc)
		//	{
		...
		public void ApplyMutation(ref Mutation mutation)
		{
			try
			{
				...
				for (var i = 0; i < mutation.FaceGenMorphSet.Count(); i++)
				{
					...
					AddMorphToggle(ref morphMutation);
		...
		private void UndoMutation(Mutation mutation)
		{
			try
			{
				...
				foreach (var mutationMorph in mutation.FaceGenMorphSet)
				{
					RemoveMorphToggle(mutationMorph.UiToggle);
					//if (!mutationMorph.Active) continue;
					UndoMutationMorph(mutationMorph);
				}

				foreach (var clothingItem in mutation.ClothingItems)
				{
					RemoveMorphToggle(clothingItem.UiToggle);
					if (!clothingItem.Active) continue;
					RemoveClothingItem(clothingItem);
				}

				foreach (var hairItem in mutation.HairItems)
				{
					RemoveMorphToggle(hairItem.UiToggle);
					if (!hairItem.Active) continue;
					RemoveHairItem(hairItem);
				}

				foreach (var item in mutation.ActiveMorphs)
				{
					RemoveMorphToggle(item.UiToggle);
					if (!item.Active) continue;
		...
		private void RemoveMorphToggle(UIDynamicToggle toggle)
		{
			try
			{
				_context.RemoveToggle(toggle);
			}
			catch (Exception e)
			{
				SuperController.LogError(e.ToString());
		...
		private void AddMorphToggle(ref MorphMutation mutationComponent)
		{
			try
			{
				var toggleData = new JSONStorableBool(mutationComponent.Name, mutationComponent.Active);
				var newToggle = _context.CreateToggle(toggleData, true);
				//_context.RegisterBool(toggleData);
				var morphMutation = mutationComponent;
				newToggle.toggle.onValueChanged.AddListener((isChecked) =>
				{
					morphMutation.Active = isChecked;
					if (isChecked) ApplyMutationMorphItem(morphMutation);
					else UndoMutationMorph(morphMutation);
					//var morph = _morphs.FirstOrDefault(m => m.displayName == mutationComponent.Name);
					//if (morph == null) return;
					//morph.morphValue = isChecked ? mutationComponent.Value : mutationComponent.PreviousValue;
				});
		...
		private void AddHairToggle(ref HairMutation mutationComponent)
		{
			try
			{
				...
				var newToggle = _context.CreateToggle(toggleData, true);
				//_context.RegisterBool(toggleData);
				var hairMutation = mutationComponent;
				newToggle.toggle.onValueChanged.AddListener((isChecked) =>
				{
					hairMutation.Active = isChecked;
					if (isChecked) ApplyHairItem(hairMutation);
					else RemoveHairItem(hairMutation);
				});
				mutationComponent.UiToggle = newToggle;
			}
			catch (Exception exc)
		...
		private void AddActiveMorphToggle(ref MorphMutation mutationComponent)
		{
			try
			{
				...
				var toggleData = new JSONStorableBool(itemName, mutationComponent.Active);
				var newToggle = _context.CreateToggle(toggleData, true);
				var item = mutationComponent;
				newToggle.toggle.onValueChanged.AddListener((isChecked) =>
				{
					item.Active = isChecked;
					if (isChecked) ApplyActiveMorphItem(item);
					else RemoveActiveMorphItem(item);
				});
				mutationComponent.UiToggle = newToggle;
			}
			catch (Exception exc)
		...
		private void AddClothingToggle(ref ClothingMutation mutationComponent)
		{
			try
			{
				...
				var newToggle = _context.CreateToggle(toggleData, true);
				//_context.RegisterBool(toggleData);
				var clothingMutation = mutationComponent;
				newToggle.toggle.onValueChanged.AddListener((isChecked) =>
				{
					clothingMutation.Active = isChecked;
					if (isChecked) ApplyClothingItem(clothingMutation);
					else RemoveClothingItem(clothingMutation);
				});
				mutationComponent.UiToggle = newToggle;
			}
			catch (Exception exc)
